...
def replace(df, col=None, ok=None, to_replace=None, value=None, inplace=False, limit=None, regex=False, method='pad'):
    df.loc[~ ok, col] = df[~ok, col].replace(to_replace=to_replace, value=value, inplace=inplace, limit=limit,
                                             regex=regex,
...
def to_int(df, col, ok, errors='coerce'):
    s = pt.to_numeric(df[col], downcast='int', errors=errors)
    return s


def to_float(df, col, ok, errors='coerce'):
